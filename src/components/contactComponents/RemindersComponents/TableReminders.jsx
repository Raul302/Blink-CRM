...
export const SlotActions = function (props) {
  ...
  function deleteReminder(id, idContact) {
    props.context.deleteReminder(id, idContact);
  }
  return (
    <>
      <td>
        {props.data.status === 'pendiente' ?
          ...
          >
            <AIcons.AiFillCheckCircle
              ...
              title="Completar"
              style={{
                color: [props.context.route == 'General' ? '#0054a3' : 'gray'],
                pointerEvents: [props.context.route == 'General' ? 'auto' : 'none']
              }}
              size={20} />
          </a>
          :
        ...
        <a className="ml-1">
          <FAIcons.FaTrashAlt title="Eliminar" style={{ color: '#FE0000' }} size={18} onClick={(e) => { deleteReminder(props.data.id, props.data.id_contact) }} />
        </a>
      </td>
    </>
  )
      ...
      .then(async (willDelete) => {
        if (willDelete) {
          if (props.route == 'General') {
            await dispatch(deleteReminderC(id, idContact))
          }
          if (props.route == 'Dashboard') {
            await dispatch(deleteReminderC(id, idContact, props.route, props.userDash.id))
          }
          if (props.route == 'Section') {
            await dispatch(deleteReminderC(id, idContact, props.route))
          }
          if (props.route == 'Prospeccion') {
            dispatch(deleteReminderC(id,idContact,props.route,null, props.activeProspect.id, 'Prospeccion'));
          }
          if (props.route == 'Aplicaciones') {
            dispatch(deleteReminderC(id,idContact,props.route,null, props.activeApplication.id, 'Aplicaciones'));
          }
          if (props.route == 'Trackings') {
            dispatch(deleteReminderC(id,idContact,props.route,null, props.activeTracking.id, 'Tracking'));
          }
